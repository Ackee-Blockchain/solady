...
from pytypes.ext.wake_tests.helpers.ERC1155Mock import ERC1155Mock, ERC1155ReceiverMock
...
def test_erc1155_events():
    ...
    assert tx.events == [
        ERC1155Mock.BeforeTokenTransfer(Address.ZERO, a.address, [0], [100], b"\x00\x11"),
        ERC1155Mock.TransferSingle(a.address, Address.ZERO, a.address, 0, 100),
        ERC1155Mock.AfterTokenTransfer(Address.ZERO, a.address, [0], [100], b"\x00\x11"),
    ]

    tx = erc1155.burn(a, 0, 100, from_=a)
    assert tx.events == [
        ERC1155Mock.BeforeTokenTransfer(a.address, Address.ZERO, [0], [100], b""),
        ERC1155Mock.TransferSingle(a.address, a.address, Address.ZERO, 0, 100),
        ERC1155Mock.AfterTokenTransfer(a.address, Address.ZERO, [0], [100], b""),
    ]

    tx = erc1155.batchMint(a, [0, 1, 2], [100, 200, 300], b"\x11\x22", from_=a)
    assert tx.events == [
        ERC1155Mock.BeforeTokenTransfer(Address.ZERO, a.address, [0, 1, 2], [100, 200, 300], b"\x11\x22"),
        ERC1155Mock.TransferBatch(a.address, Address.ZERO, a.address, [0, 1, 2], [100, 200, 300]),
        ERC1155Mock.AfterTokenTransfer(Address.ZERO, a.address, [0, 1, 2], [100, 200, 300], b"\x11\x22"),
    ]

    tx = erc1155.batchBurn(a, [0, 1, 2], [100, 200, 300], from_=a)
    assert tx.events == [
        ERC1155Mock.BeforeTokenTransfer(a.address, Address.ZERO, [0, 1, 2], [100, 200, 300], b""),
        ERC1155Mock.TransferBatch(a.address, a.address, Address.ZERO, [0, 1, 2], [100, 200, 300]),
        ERC1155Mock.AfterTokenTransfer(a.address, Address.ZERO, [0, 1, 2], [100, 200, 300], b""),
    ]

    erc1155.mint(a, 0, 100, b"", from_=a)
    tx = erc1155.safeTransferFrom(a, b, 0, 100, b"", from_=a)
    assert tx.events == [
        ERC1155Mock.BeforeTokenTransfer(a.address, b.address, [0], [100], b""),
        ERC1155Mock.TransferSingle(a.address, a.address, b.address, 0, 100),
        ERC1155Mock.AfterTokenTransfer(a.address, b.address, [0], [100], b""),
    ]

    tx = erc1155.safeTransferFrom(b, b, 0, 100, b"\x33", from_=b)
    assert tx.events == [
        ERC1155Mock.BeforeTokenTransfer(b.address, b.address, [0], [100], b"\x33"),
        ERC1155Mock.TransferSingle(b.address, b.address, b.address, 0, 100),
        ERC1155Mock.AfterTokenTransfer(b.address, b.address, [0], [100], b"\x33"),
    ]

    erc1155.setApprovalForAll(a, True, from_=b)
    tx = erc1155.safeTransferFrom(b, a, 0, 100, b"", from_=a)
    assert tx.events == [
        ERC1155Mock.BeforeTokenTransfer(b.address, a.address, [0], [100], b""),
        ERC1155Mock.TransferSingle(a.address, b.address, a.address, 0, 100),
        ERC1155Mock.AfterTokenTransfer(b.address, a.address, [0], [100], b""),
    ]
    ...
    assert tx.events == [
        ERC1155Mock.BeforeTokenTransfer(a.address, b.address, [0, 0, 0], [35, 30, 35], b""),
        ERC1155Mock.TransferBatch(b.address, a.address, b.address, [0, 0, 0], [35, 30, 35]),
        ERC1155Mock.AfterTokenTransfer(a.address, b.address, [0, 0, 0], [35, 30, 35], b""),
    ]
    ...
    assert tx.events == [
        ERC1155Mock.BeforeTokenTransfer(b.address, Address.ZERO, [0], [100], b""),
        ERC1155Mock.TransferSingle(a.address, b.address, Address.ZERO, 0, 100),
        ERC1155Mock.AfterTokenTransfer(b.address, Address.ZERO, [0], [100], b""),
    ]
    ...
    assert tx.events == [
        ERC1155Mock.BeforeTokenTransfer(a.address, Address.ZERO, [2], [100], b""),
        ERC1155Mock.TransferSingle(c.address, a.address, Address.ZERO, 2, 100),
        ERC1155Mock.AfterTokenTransfer(a.address, Address.ZERO, [2], [100], b""),
    ]

    tx = erc1155.batchBurnUnchecked(Address.ZERO, a, [0, 1, 2], [100, 100, 100], from_=c)
    assert tx.events == [
        ERC1155Mock.BeforeTokenTransfer(a.address, Address.ZERO, [0, 1, 2], [100, 100, 100], b""),
        ERC1155Mock.TransferBatch(c.address, a.address, Address.ZERO, [0, 1, 2], [100, 100, 100]),
        ERC1155Mock.AfterTokenTransfer(a.address, Address.ZERO, [0, 1, 2], [100, 100, 100], b""),
    ]

    tx = erc1155.safeTransferUnchecked(Address.ZERO, a, b, 1, 50, b"\x11", from_=c)
    assert tx.events == [
        ERC1155Mock.BeforeTokenTransfer(a.address, b.address, [1], [50], b"\x11"),
        ERC1155Mock.TransferSingle(c.address, a.address, b.address, 1, 50),
        ERC1155Mock.AfterTokenTransfer(a.address, b.address, [1], [50], b"\x11"),
    ]

    tx = erc1155.safeBatchTransferUnchecked(Address.ZERO, a, b, [0, 1, 2], [0, 50, 0], b"\x22", from_=c)
    assert tx.events == [
        ERC1155Mock.BeforeTokenTransfer(a.address, b.address, [0, 1, 2], [0, 50, 0], b"\x22"),
        ERC1155Mock.TransferBatch(c.address, a.address, b.address, [0, 1, 2], [0, 50, 0]),
        ERC1155Mock.AfterTokenTransfer(a.address, b.address, [0, 1, 2], [0, 50, 0], b"\x22"),
    ]
